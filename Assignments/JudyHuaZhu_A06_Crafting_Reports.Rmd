...
```{r project.setup, include=FALSE,echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
knitr::kable(summary_PM25,caption = "Mean Particulates (2.5mm)",col.names = c("County","$\\mu g/m^3$"),format.args = list(digits = 2,nsmall = 2))

knitr::kable(summary_Ozone,caption = "Mean Ozone",col.names = c("County","$\\mu g/m^3$"),format.args = list(digits = 2,nsmall = 2))
```
...
```{r, fig.cap = "2018 monthly ozone level", echo=FALSE, fig.width = 13, fig.height = 6, fig.align = 'left'}
Ozone_18<- epa_data %>% 
  drop_na(Ozone) %>% 
  filter(Year == 2018)

Ozone_18$Month <- factor(Ozone_18$Month,
       levels=1:12,
       labels = month.abb)
ggplot(data = Ozone_18, aes(x = Month,y= Ozone)) +
  geom_boxplot()+
  ggtitle ("Monthly Ozone concentrations (2018)")
```

```{r, fig.cap = "2019 monthly ozone level", echo=FALSE, fig.width = 13, fig.height = 6, fig.align = 'left'}
Ozone_19<- epa_data %>% 
  drop_na(Ozone) %>% 
  filter(Year == 2019)

Ozone_19$Month <- factor(Ozone_19$Month,
       levels=1:12,
       labels = month.abb)
ggplot(data = Ozone_19, aes(x = Month,y= Ozone)) +
  geom_boxplot()+
