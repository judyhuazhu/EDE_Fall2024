...
setwd('./Data') #capitalization counts 
getwd()
here()
...
filename <- here('Data','Raw','USGS_Site02085000_Flow_Raw.csv') #'./means current directory'
print(filename)
...
#Create and print a list of all files in a folder using "dir" and "here"; "dir" means directory
filenames <- dir(here('Data','Raw'),pattern = ("*.csv")) #anything end with .csv
print(filenames)
```
...
date_obj_today <- ymd(str_today) #convert the string to date object
date_obj_today
...
fix.early.dates <- function(d, cutoff) {
       #d is any date, cutoff is current year
       m <- year(d) %% 100  
...
fixed_date_obj_past <- fix.early.dates(date_obj_past,24) 
...
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE) #don't drop empty value
