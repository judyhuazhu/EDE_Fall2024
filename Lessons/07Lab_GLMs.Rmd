...
#Exercise 1:
#1 
density.O3 <- ggplot(EPAair, aes(x = Ozone, fill=as.factor(Year)))+
...
#2 
boxplot.O3 <- ggplot(EPAair, aes(y = Ozone, x = as.factor(Year)) )+
         geom_boxplot()+
        ylim(0.150)
...
PM2.5byOzone.scatterplot <- ggplot(EPAair, aes(x=Ozone,y=PM2.5))+
  geom_point()+
  geom_smooth(method="lm", col = "blue")=
  geom_text(x=100,y=75,label = expression ("PM2.5=0.38*Ozone+15.64"))

print(PM2.5byOzone.scatterplot)
...
library(corrplot)
EPA.Lat.Ozone <- EPAair %>% 
  select(mean_Lat:Ozone) %>% 
  na.omit()

EPAirCor <- cor(EPA.Lat.Ozone)
corrplot(EPAirCor, upper = "ellipse")
