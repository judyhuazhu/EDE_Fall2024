...
# Some packages are umbrellas/compound under which other packages are loaded
...
USGS.flow.data <- read.csv(
  ...
  # all characters becomes factor, treated by categorical values instead of strings
  stringsAsFactors = TRUE)   
...
```{r Exploring data}
# View(USGS.flow.data) #comment out before knit
...
str(USGS.flow.data) #structure, to overview the amount of data, data types
dim(USGS.flow.data) #demension
length(USGS.flow.data) # no of rows

head(USGS.flow.data)
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #specific rows and columns

class(USGS.flow.data$datetime)
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

#choose all the numeric columns and give stats (might be weird values)
...
```{r Removing NAs} 
USGS.flow.data.complete <- na.omit(USGS.flow.data) #delete the rows with missing data
dim(USGS.flow.data)
dim(USGS.flow.data.complete)

#mean(USGS.flow.data$discharge.mean) #cannot run with missing data
